...
import math # import math object
import sys
from pygame import mixer
pygame.init() # This is wehn pygame is would initalization
pygame.font.init() # This is set the background of pygame

#This is to allow the game to be seen in detail for its simplisity
DISPLAY_WIDTH = 800
DISPLAY_HEIGHT = 600
gameDisplay = pygame.display.set_mode((DISPLAY_WIDTH,DISPLAY_HEIGHT))
pygame.display.set_caption('Marcos Tetris ') #  This is the name of the screen
...
#Font sizes
#Each font correspond to the names, title, start, etc.
SB_FONT_SIZE = 29 
FONT_SIZE_SMALL = 17 
PAUSE_FONT_SIZE = 66
...
#The grey-scale color scheme belongs to the walls (little refference to tetris gameboy)
BLACK = (0,0,0)
WHITE = (255,255,255)
DARK_GRAY = (80,80,80)
GRAY = (110,110,110)
...
NUM_COLOR = WHITE
TEXT_COLOR = GRAY
# Name of the block of color
#Each color block is easy to select, each is unique and easy to tell
blockColors = {
'I' : (19,232,232), #CYAN
'O' : (236,236,14), #YELLOW
'T' : (126,5,126), #PURPLE
...
# This are all the type of position the block are going to make
pieceDefs = {
'I' : ((1,0),(1,1),(1,2),(1,3)),
'O' : ((0,1),(0,2),(1,1),(1,2)),
'T' : ((0,1),(1,0),(1,1),(1,2)),
...
#This is basic coding numbers
#This step lets the player move the blocks left and right
directions = {
'down' : (1,0),
'right' : (0,1),
'left' : (0,-1),
...
# Background Music
#
#mixer.music.load('Tetris example 4.wav') # This is my music
#mixer.music.play(-1) # This is my music
#BackGround theme of the game
#This is the background theme, as mention to make intriguing music
#win = pygame.display.set_mode((1000,500))
#The jpg is an image provided by Dark_man 20 for the background
#BackgroundTheme = pygame.image.load('pexels-guillaume-meurice-2873671.jpg')
#bg = pygame.transform.scale(BackgroundTheme, (1000, 500))
#p = 0
...
#This allow the player to move the piece and around
class GameKeyInput:
...
#This is big chunk of the game that is important via cloak system
class GameClock:
...
class MainBoard:
	...
	# Title of the game and more
	def draw_GAMEBOARD_CONTENT(self):
	
		if self.gameStatus == 'firstStart':	

			titleText = fontTitle.render('TETRIS', False, WHITE)   # Default screen
			gameDisplay.blit(titleText,(self.xPos++1.55*self.blockSize,self.yPos+8*self.blockSize))
			titleText = fontTitle.render('Marco', False, WHITE)  # My Name
			gameDisplay.blit(titleText,(self.xPos++1.55*self.blockSize,self.yPos+5*self.blockSize))
			versionText = fontVersion.render('All right are reseverd to Marco', False, WHITE) # Right reseverd to Marco
			gameDisplay.blit(versionText,(self.xPos++0.3*self.blockSize,self.yPos+14*self.blockSize))
			versionText = fontVersion.render('Todo esto ta resevado a Marcos ', False, WHITE) # Right reseverd to Marco
			gameDisplay.blit(versionText,(self.xPos++0.3*self.blockSize,self.yPos+16*self.blockSize))
	...
	while not gameExit: #Stay in this loop unless the game is quit
		...
		for event in pygame.event.get(): # For loop\
			...
			if event.type == pygame.QUIT: #Looks for quitting event in every iteration (Meaning closing the game window)
				gameExit = True # boolean player if want to extis the game



			if event.type == pygame.KEYDOWN: #Keyboard keys press events
				if event.key == pygame.K_LEFT: # keyword geos down
					xChange += -1
...
pygame.quit()
sys.exit()
#From here the game should work and allow everyone to play